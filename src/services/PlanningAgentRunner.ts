...
export class PlanningAgentRunner {
    ...
    private activeHeartbeats: Map<string, NodeJS.Timeout> = new Map();  // Track heartbeat intervals for cleanup
...
`;
    ...
    /**
     * Run Context Gatherer agent in background with retry logic
     */
    private runContextGathererAgent(
        ...
        this.runContextGathererAgentWithRetry(processId, prompt, contextFile, onProgress, 0, maxRetries);
    }

    /**
     * Internal retry implementation for Context Gatherer
     */
    private runContextGathererAgentWithRetry(
        processId: string,
        prompt: string,
        contextFile: string,
        onProgress?: (message: string) => void,
        ...
        const attemptId = attempt > 0 ? `${processId}_retry${attempt}` : processId;
        
        if (attempt > 0) {
            onProgress?.(`[Context Gatherer] 🔄 Retry ${attempt}/${maxRetries}...`);
        }
        
        // Write prompt to temp dir to avoid command line length issues (and keep project clean)
        const promptFile = path.join(os.tmpdir(), `apc_context_gatherer_${Date.now()}.txt`);
        ...
        console.log(`[AgentRunner] Started Context Gatherer process PID: ${proc.pid}, tracked as: ${attemptId}`);
        ...
        proc.stdout?.on('data', (data) => {
            ...
            const heartbeat = setInterval(() => {
                if (this.activeProcesses.has(processId)) {
                    ...
                    this.activeHeartbeats.delete(attemptId);
                }
            }, 15000);
            this.activeHeartbeats.set(attemptId, heartbeat);
            ...
            proc.on('close', (code) => {
                ...
                if (code !== 0 && attempt < maxRetries) {
                    onProgress?.(`[Context Gatherer] ⚠️ Failed (code ${code}), will retry...`);
                    setTimeout(() => {
                        this.runContextGathererAgentWithRetry(
                            processId, prompt, contextFile, onProgress, attempt + 1, maxRetries
                        );
                    }, 2000);
                    return;
                }
            ...
            proc.on('error', (err) => {
                ...
                if (attempt < maxRetries) {
                    onProgress?.(`[Context Gatherer] ❌ Error: ${err.message}, will retry...`);
                    setTimeout(() => {
                        this.runContextGathererAgentWithRetry(
                            processId, prompt, contextFile, onProgress, attempt + 1, maxRetries
                        );
                ...
                onProgress?.(`[Context Gatherer] ❌ Failed after ${maxRetries} retries: ${err.message}`);
            });
...
`);
        ...
        return this.runTaskReviewWithRetry(processId, prompt, contextFile, onProgress, 0, maxRetries);
    }

    /**
     * Internal retry implementation for Task Review
     */
    private runTaskReviewWithRetry(
        processId: string,
        prompt: string,
        contextFile: string,
        onProgress?: (message: string) => void,
        ...
        if (attempt > 0) {
            onProgress?.(`[Task Review] 🔄 Retry ${attempt}/${maxRetries}...`);
        }

        return new Promise((resolve) => {
            // Write prompt to temp dir to avoid command line length issues (and keep project clean)
            const promptFile = path.join(os.tmpdir(), `apc_task_review_${Date.now()}.txt`);
            fs.writeFileSync(promptFile, prompt);
            ...
            this.activeProcesses.set(attemptId, proc);
            console.log(`[AgentRunner] Started Task Review process PID: ${proc.pid}, tracked as: ${attemptId}`);
            ...
            proc.on('close', (code) => {
                ...
                // RETRY on failure if we haven't exceeded max retries
                if (code !== 0 && attempt < maxRetries) {
                    onProgress?.(`[Task Review] ⚠️ Failed (code ${code}), will retry...`);
                    this.runTaskReviewWithRetry(
                        processId, prompt, contextFile, onProgress, attempt + 1, maxRetries
                    ).then(resolve);
                    return;
                }
            ...
            proc.on('error', (err) => {
                ...
                // RETRY on error if we haven't exceeded max retries
                if (attempt < maxRetries) {
                    onProgress?.(`[Task Review] ❌ Error: ${err.message}, will retry...`);
                    this.runTaskReviewWithRetry(
                        processId, prompt, contextFile, onProgress, attempt + 1, maxRetries
                    ).then(resolve);
                    return;
                ...
                onProgress?.(`[Task Review] ❌ Failed after ${maxRetries} retries: ${err.message}`);
                resolve(contextFile);
            });
            ...
            setTimeout(() => {
                if (this.activeProcesses.has(attemptId)) {
                    ...
                    if (attempt < maxRetries) {
                        onProgress?.(`[Task Review] ⏰ Timeout, will retry...`);
                        this.runTaskReviewWithRetry(
                            processId, prompt, contextFile, onProgress, attempt + 1, maxRetries
                        ).then(resolve);
                        return;
                    ...
                    onProgress?.(`[Task Review] ⏰ 10min timeout reached after ${maxRetries} retries`);
                    resolve(contextFile);
                }
            }, 600000); // 10 minutes
        });
...
\`\`\`bash
cat > $TMPDIR/${analyst.id}_analysis.md << 'ANALYSIS_EOF'
**Status:** ✅ Complete
...
`;
    ...
    async runMultiAgentDebate(
        ...
        if (!this.isCursorAvailable()) {
            onProgress?.('❌ Cursor CLI not available. Please install Cursor CLI to use multi-agent planning.');
            throw new Error('Cursor CLI not available. Install Cursor and ensure "cursor" is in PATH.');
        }
        ...
        const MAX_RETRIES = 2;
        let retryCount = 0;
        let analystsToRun = [...ANALYST_CONFIGS];
        let analyses: AgentAnalysis[] = [];

        while (analystsToRun.length > 0 && retryCount <= MAX_RETRIES) {
            // Phase 1: Run analysts in parallel - they write to plan file
            onProgress?.(`\n═══════════════════════════════════════════`);
            onProgress?.(`  PHASE 1: PARALLEL ANALYSIS (${analystsToRun.length} agents)${retryCount > 0 ? ` [Retry ${retryCount}]` : ''}`);
            onProgress?.(`═══════════════════════════════════════════`);

            const analystPromises = analystsToRun.map(analyst => 
                this.runAnalystSession(analyst, requirement, docs, planFile, contextSummary, onProgress, poolSize)
                    .catch(err => {
                        onProgress?.(`⚠️ ${analyst.name} failed: ${err.message}`);
                        return null;  // Don't reject the whole Promise.all
                    })
            );
            ...
            // Phase 2: Read plan file and check which analysts completed
            onProgress?.(`\n═══════════════════════════════════════════`);
            onProgress?.(`  PHASE 2: CHECKING ANALYST RESULTS`);
            onProgress?.(`═══════════════════════════════════════════`);
            ...
            analyses = this.parsePlanFileAnalysesInternal(planContent);
            
            onProgress?.(`📊 Parsed ${analyses.length}/${ANALYST_CONFIGS.length} analyst contributions`);

            // Find analysts that didn't complete
            const completedIds = new Set(analyses.map(a => 
                ANALYST_CONFIGS.find(c => c.name === a.agentName)?.id
            ).filter(Boolean));

            analystsToRun = ANALYST_CONFIGS.filter(a => !completedIds.has(a.id));

            if (analystsToRun.length > 0) {
                retryCount++;
                if (retryCount <= MAX_RETRIES) {
                    onProgress?.(`🔄 ${analystsToRun.length} analyst(s) didn't complete: ${analystsToRun.map(a => a.name).join(', ')}`);
                    onProgress?.(`🔄 Retrying (attempt ${retryCount}/${MAX_RETRIES})...`);
                } else {
                    onProgress?.(`⚠️ ${analystsToRun.length} analyst(s) failed after ${MAX_RETRIES} retries`);
                }
            }
        }
        ...
        if (analyses.length === 0) {
            throw new Error(`All ${ANALYST_CONFIGS.length} analysts failed after ${MAX_RETRIES} retries. Check Cursor CLI and network.`);
        }

        onProgress?.(`✅ ${analyses.length} analyst(s) completed successfully`);
     ...
     * 
     * Note: This is the internal version that doesn't throw on empty results.
     * The retry logic in runMultiAgentDebate handles empty results.
     */
    private parsePlanFileAnalysesInternal(planContent: string): AgentAnalysis[] {
...
**Rationale:** Based on ${analyses.length} analyst votes and task parallelism analysis`;
    ...
    private async runAnalystSession(
        ...
        return new Promise((resolve, reject) => {
            // Write prompt to temp dir to avoid command line length issues (and keep project clean)
            const promptFile = path.join(os.tmpdir(), `apc_analyst_${analyst.id}_${Date.now()}.txt`);
            fs.writeFileSync(promptFile, prompt);
            ...
            const heartbeat = setInterval(() => {
                if (this.activeProcesses.has(processId)) {
                    ...
                    clearInterval(heartbeat);
                    this.activeHeartbeats.delete(processId);
                }
            }, 20000);
            this.activeHeartbeats.set(processId, heartbeat);
    ...
    // 
    // Mock analysis fallback was also removed - Cursor CLI is required for planning.
    // If Cursor CLI is not available, runMultiAgentDebate will throw an error.
    ...
    async dispose(): Promise<void> {
        console.log('[AgentRunner] Disposing...');
        
        // Stop all active processes
        await this.stopAll();
        
        // Clear all heartbeat intervals
        for (const [id, interval] of this.activeHeartbeats) {
            clearInterval(interval);
            console.log(`[AgentRunner] Cleared heartbeat for ${id}`);
        }
        this.activeHeartbeats.clear();
        
        // Clear context gatherer
        if (this.contextGatherer) {
            await this.stopContextGatherer();
        }
