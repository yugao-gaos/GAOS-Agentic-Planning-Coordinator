...
export interface IStateManagerApi {
    getAllPlanningSessions(): any[];
    getPlanningSession(id: string): any | undefined;
}
...
export interface IAgentPoolApi {
    ...
    getAllRoles(): Array<{ id: string; name: string; description: string; isBuiltIn: boolean; defaultModel: string; timeoutMs: number }>;
    getRole(roleId: string): any | undefined;
...
export interface IRoleRegistryApi {
    getRole(roleId: string): any | undefined;
    updateRole(roleId: string, updates: Record<string, any>): void;
...
export interface ICoordinatorApi {
    ...
    getWorkflowStatus(sessionId: string, workflowId: string): any | null;
...
export interface IPlanningApi {
    listPlanningSessions(): any[];
    getPlanningStatus(id: string): any | null;
    startPlanning(prompt: string, docs?: string[]): Promise<{
        sessionId: string;
        status: string;
        planPath?: string;
...
export class ApiHandler {
     ...
     */
    private async routeCommand(cmd: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
    ...
    private async handleSession(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        switch (action) {
            case 'list':
                return { data: this.sessionList() };
            
            case 'status':
                return { data: this.sessionStatus(params.id) };
            
            case 'pause':
                await this.services.coordinator.pauseSession(params.id);
    ...
    private async handlePlan(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        switch (action) {
            case 'list':
            ...
            case 'status':
                const status = this.services.planningService.getPlanningStatus(params.id);
                if (!status) {
    ...
    private async handleExec(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        const sessionId = params.sessionId || params.session || params.id;
    ...
    private async handleWorkflow(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        switch (action) {
            case 'dispatch':
            ...
            case 'status':
                const progress = this.services.coordinator.getWorkflowStatus(params.sessionId, params.workflowId);
                if (!progress) {
            ...
            case 'cancel':
                await this.services.coordinator.cancelWorkflow(params.sessionId, params.workflowId);
                return { message: `Workflow ${params.workflowId} cancelled` };
            
            case 'list':
                const workflows = this.services.coordinator.getWorkflowSummaries(params.sessionId);
    ...
    private async handlePool(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        switch (action) {
            case 'status':
            case undefined:
            ...
            case 'resize':
                const result = this.services.agentPoolService.resizePool(params.size);
                return {
    ...
    private async handleAgent(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        switch (action) {
            case 'pool':
            ...
            case 'release':
                this.services.agentPoolService.releaseAgents([params.agentName]);
     ...
     */
    private agentComplete(params: Record<string, any>): { data?: any; message?: string } {
        const { session, workflow, stage, result, data } = params;
        
        // Validate required params
        if (!session) throw new Error('Missing session parameter');
        if (!workflow) throw new Error('Missing workflow parameter');
        if (!stage) throw new Error('Missing stage parameter');
        if (!result) throw new Error('Missing result parameter');
    ...
    private async handleTask(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        ...
        switch (action) {
            case 'progress':
                if (!sessionId) throw new Error('Missing session parameter');
                return { data: this.taskProgress(sessionId) };
            
            case 'status':
                if (!sessionId) throw new Error('Missing session parameter');
                if (!params.task && !params.taskId) throw new Error('Missing task parameter');
                return { data: this.taskStatus(sessionId, params.task || params.taskId) };
            
            case 'fail':
                if (!sessionId) throw new Error('Missing session parameter');
                if (!params.task && !params.taskId) throw new Error('Missing task parameter');
                if (!params.reason) throw new Error('Missing reason parameter');
                return this.taskFail(sessionId, params.task || params.taskId, params.reason);
    ...
    private taskProgress(sessionId: string): any {
    ...
    private taskStatus(sessionId: string, taskId: string): any {
    ...
    private taskFail(sessionId: string, taskId: string, reason: string): { data?: any; message?: string } {
        const globalTaskId = `${sessionId}_${taskId}`;
    ...
    private unityStatus(): UnityStatusResponse {
        ...
        return {
            ...
            isPlaying: (state as any).isPlaying || false,
            isCompiling: (state as any).isCompiling || false,
            hasErrors: (state as any).hasErrors || false,
            errorCount: (state as any).errorCount || 0,
    ...
    private async handleRoles(action: string, params: Record<string, any>): Promise<{ data?: any; message?: string }> {
        ...
        switch (action) {
            ...
            case 'get':
                if (!params.roleId) throw new Error('Missing roleId parameter');
                const role = this.services.roleRegistry.getRole(params.roleId);
                if (!role) throw new Error(`Role ${params.roleId} not found`);
                return { data: role };
            
            case 'update':
                if (!params.roleId) throw new Error('Missing roleId parameter');
                if (!params.updates) throw new Error('Missing updates parameter');
                this.services.roleRegistry.updateRole(params.roleId, params.updates);
                const updatedRole = this.services.roleRegistry.getRole(params.roleId);
                return { 
                    data: updatedRole,
            ...
            case 'reset':
                if (!params.roleId) throw new Error('Missing roleId parameter');
                const wasReset = this.services.roleRegistry.resetRoleToDefault(params.roleId);
                if (!wasReset) {
                    throw new Error(`Role ${params.roleId} is not a built-in role or not found`);
                }
                const resetRole = this.services.roleRegistry.getRole(params.roleId);
                return { 
                    data: resetRole,
